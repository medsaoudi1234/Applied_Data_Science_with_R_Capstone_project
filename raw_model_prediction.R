...
get_weather_forecaset_by_cities <- function(city_names){
  ...
  for (city_name in city_names){
    ...
    api_key <- "0016712b9825b2e69c4e5d6cedd9382b"
    ...
    for(result in results) {      
      ...
      weather_detail_label <- paste(sep = "",
                                    ...
                                    "Visibility: ", ifelse("visibility" %in% names(result), result$visibility, "NA"), " m </br>",  # Display visibility data or "NA" if missing
  ...
  weather_df <- tibble(CITY_ASCII=city, WEATHER=weather,
                       ...
                       HUMIDITY=humidity,
                       WIND_SPEED=wind_speed, SEASONS=seasons, HOURS=hours,
...
predict_bike_demand <- function(TEMPERATURE, HUMIDITY, WIND_SPEED, VISIBILITY, SEASONS, HOURS){
  model <- load_saved_model("/home/rstudio/Test it/Final project/model.csv")
...
generate_city_weather_bike_data <- function (){
  ...
  cities_bike_pred <- cities_df %>% left_join(results) %>%
    select(CITY_ASCII, LNG, LAT, TEMPERATURE, HUMIDITY, BIKE_PREDICTION, BIKE_PREDICTION_LEVEL,
